...
const E_OUTOFMEMORY: HRESULT = 0x80007000e;
const E_NOTIMPL: HRESULT = 0x80004001;
// library errors
const E_BUFFERTOOSMALL: HRESULT = 0xff04006f;
const E_PAGENOTACTIVE: HRESULT = 0xff040001;
...
directoutputlib_export! {
    fn DirectOutput_Initialize(app_name: *const widestring::WideChar) -> HRESULT {
        let mut state = STATE.lock().unwrap();
        if state.is_none() {
            state.replace(devices::init().unwrap());
        }
        return S_OK
    }
}
...
directoutputlib_export! {
    fn DirectOutput_Enumerate(callback: Pfn_DirectOutput_EnumerateCallback, prg_ctx: PrgCtx) -> HRESULT {
        ...
        let state: &devices::State = match *guard {
            Some(ref x) => x,
...
directoutputlib_export! {
    fn DirectOutput_SetProfile(device_ptr: DevicePtr, debug_profile_name_size: usize, debug_profile_name: *mut widestring::WideChar) -> HRESULT {
        // TODO
        return E_NOTIMPL;
    }
}
...
directoutputlib_export! {
    fn DirectOutput_SetString(device_ptr: DevicePtr, page_number: DWORD, string_index: DWORD, string_size: DWORD, string: *const widestring::WideChar) -> HRESULT {
        // TODO
        return S_OK;
    }
}
...
directoutputlib_export! {
    fn DirectOutput_SetImageFromFile(device_ptr: DevicePtr, page_number: DWORD, image_index: DWORD, filename_size: DWORD, filename: *const widestring::WideChar) -> HRESULT {
        // TODO
        return S_OK;
    }
}
...
directoutputlib_export! {
    fn DirectOutput_GetSerialNumber(device_ptr: DevicePtr, res_serial_number: *mut widestring::WideChar, res_serial_number_size: usize) -> HRESULT {
        let guard = STATE.lock().unwrap();
        let state: &devices::State = match *guard {
            Some(ref x) => x,
            None => return E_HANDLE,
        ...
        unsafe { widestring::WideCString::from_ptr_unchecked(res_serial_number, res_serial_number_size).as_mut_slice() }.copy_from_slice(
